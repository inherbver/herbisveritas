  ...
  if (!userId) {
    try {
      ...
      if (
        ...
        error.message?.includes("Database error creating anonymous user")
      ) {
  ...
  if (pathToCheck.startsWith("/admin")) {
    if (!user) {
      return NextResponse.redirect(
        `/${currentLocale}/login?redirectUrl=${encodeURIComponent(pathname)}`
      );
    }
...
export async function authAction(prevState: any, formData: FormData): Promise<ActionResult<T>> {
...
**🔐 Système d'Authentification Herbis Veritas**  
_Sécurisé • Scalable • Production-Ready_
