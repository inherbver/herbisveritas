...
const UserProfileSchema = z
  .object({
    ...
    email: z.string().email("Email invalide").max(255, "Email trop long").toLowerCase(),
...
export const withRateLimit = (
  ...
  return async (req: NextRequest) => {
    ...
    const { allowed, remaining } = await rateLimiter.checkLimit(key, config.limit, config.window);
...
class SecurityAuditLogger {
  ...
  private getSeverityLevel(eventType: string): "low" | "medium" | "high" | "critical" {
    ...
    const highEvents = ["ADMIN_ROLE_CHANGE", "BULK_DATA_EXPORT", "SENSITIVE_DATA_ACCESS"];
...
class GDPRService {
  ...
  async updateUserData(userId: string, corrections: DataCorrections): Promise<void> {
    ...
    await this.supabase.from("profiles").update(validatedData).eq("id", userId);
