-- Migration critique : Index de performance manquants
-- Date : 2025-08-19
-- Impact attendu : -40% à -80% temps de réponse pages critiques
-- Risque : Aucun (utilisation CONCURRENTLY)
...
-- Vue pour monitorer l'usage des nouveaux index
...
COMMENT ON INDEX idx_products_active_created IS 
'Index critique pour page boutique - tri par date des produits actifs';

COMMENT ON INDEX idx_products_search IS 
'Index full-text pour recherche produits en français';

COMMENT ON INDEX idx_cart_items_cart_product IS 
'Index composite pour optimiser requêtes panier checkout';

COMMENT ON INDEX idx_orders_admin_dashboard IS 
'Index optimisé pour dashboard admin avec données incluses';

COMMENT ON INDEX idx_profiles_auth_lookup IS 
'Index authentification avec données utilisateur incluses';

-- =======================================
-- VALIDATION POST-MIGRATION
-- =======================================

-- Vérifier que tous les index ont été créés avec succès
DO $$
DECLARE
    index_count INTEGER;
BEGIN
    ...
    IF index_count >= 11 THEN
        RAISE NOTICE 'SUCCESS: % index critiques créés avec succès', index_count;
    ELSE
        RAISE WARNING 'ATTENTION: Seulement % index créés sur 11 attendus', index_count;
    END IF;
END $$;