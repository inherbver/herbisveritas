...
class RateLimitingApplicator {
  ...
  /**
   * Getter pour accéder aux actions
   */
  public getActions(): ServerAction[] {
    return this.actions;
  }
   ...
   */
  public async scanServerActions(): Promise<void> {
  ...
  private categorizeAction(
    ...
    if (
      ...
      ["login", "signup", "logout", "resetpassword", "forgotpassword"].some(
        (auth) => actionName.includes(auth),
      )
    ) {
      return "AUTH";
    }
    ...
    if (
      ...
      ["checkout", "payment", "stripe"].some((payment) =>
    ...
    if (
      ...
      ["setuserrole", "deleteuser", "admin"].some((admin) =>
        actionName.includes(admin),
      )
    ) {
      return "ADMIN";
    }
    ...
    // Actions de panier
    if (
      fileName.includes("cart") ||
      ["additem", "removeitem", "updatequantity", "cart"].some((cart) =>
        actionName.includes(cart),
      )
    ) {
      return "CART";
    }
    ...
    // Actions de contenu
    if (
      ["create", "update", "delete", "upload", "publish"].some((content) =>
        actionName.includes(content),
      )
    ) {
      return "CONTENT";
    }
  ...
  private async applyRateLimitingToActions(): Promise<ServerAction[]> {
    ...
    const unprotectedActions = this.actions.filter(
      (action) => !action.hasRateLimit,
    );

    console.log(
      `🛡️  Application du rate limiting à ${unprotectedActions.length} actions...`,
    );
    ...
    for (const action of unprotectedActions) {
      try {
        ...
        this.errors.push(
          `Erreur application rate limiting ${action.functionName}: ${error}`,
        );
      }
    }
   ...
   */
  private async applyRateLimitingToFile(
    action: ServerAction,
  ): Promise<boolean> {
    try {
      ...
      const functionPatterns = [
        {
          pattern: new RegExp(
            `export\\s+async\\s+function\\s+${action.functionName}\\s*\\(`,
          ),
        ...
        {
          pattern: new RegExp(
            `export\\s+const\\s+${action.functionName}\\s*=\\s*async\\s+function`,
          ),
          replacement: `export const ${action.functionName} = withRateLimit(\n  '${action.category}',\n  '${this.kebabCase(action.functionName)}'\n)(async function`,
        },
      ];
      ...
      console.log(
        `✅ Rate limiting appliqué: ${action.functionName} (${action.category})`,
      );
      return true;
    } catch (error) {
  ...
  private generateReport(appliedActions: ServerAction[]): RateLimitingReport {
    const protectedActions = this.actions.filter(
      (action) => action.hasRateLimit,
    );
    const unprotectedActions = this.actions.filter(
      (action) => !action.hasRateLimit,
...
export class RateLimitingValidator {
  ...
  static async validateRateLimiting(): Promise<{
    ...
    const actions = applicator.getActions();
    const protectedActions = actions.filter((action) => action.hasRateLimit);
    const coverage =
      actions.length > 0 ? (protectedActions.length / actions.length) * 100 : 0;
    ...
    if (coverage < 80) {
      issues.push(
        `Couverture insuffisante: ${coverage.toFixed(1)}% (minimum 80%)`,
      );
    }
    ...
    const unprotectedCritical = criticalActions.filter(
      (action) => !action.hasRateLimit,
    );

    if (unprotectedCritical.length > 0) {
      issues.push(
        `Actions critiques non protégées: ${unprotectedCritical.map((a) => a.functionName).join(", ")}`,
      );
    }
...
async function main() {
  try {
    ...
    console.log(
      `Couverture: ${((report.protectedActions / report.totalActions) * 100).toFixed(1)}%`,
    );
    ...
    if (report.unprotectedActions.length > 0) {
      ...
      report.unprotectedActions.forEach((action) => {
        console.log(
          `  - ${action.functionName} (${action.category}) - ${action.filePath}`,
        );
      });
    }
