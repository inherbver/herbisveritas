...
const DisplayAddress = ({
  ...
  if (!address) {
    const noAddressMessage = addressTypeLabel === t('shippingAddressTitle')
      ? t('noShippingAddress')
      : t('noBillingAddress');
  ...
  return (
    <div className="p-4 border rounded-lg shadow-sm bg-white space-y-1 relative">
      {address.full_name && <p className="font-semibold text-lg">{address.full_name}</p>}
      ...
      {address.phone_number && <p>{t('phone')}: {address.phone_number}</p>}
      <button
        onClick={onEdit}
...
export default function AddressesPage({ params: { locale } }: Props) {
  ...
  const [editingAddress, setEditingAddress] = useState<(Partial<AddressFormData> & { id?: string }) | null>(null);
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        if (fetchError) {
          ...
          setShippingAddress(
            addressesData.find((addr: Address) => addr.address_type === 'shipping' && addr.is_default) ||
            addressesData.find((addr: Address) => addr.address_type === 'shipping') ||
            null
          );
          setBillingAddress(
            addressesData.find((addr: Address) => addr.address_type === 'billing' && addr.is_default) ||
  ...
  const handleFormSuccess = async () => {
    ...
    try {
      ...
      console.error('Unexpected error re-fetching addresses:', error.message);
    }
    setLoading(false);
  };
  ...
  if (!user) {
    return <p className="text-center py-10">Please log in to view your addresses.</p>;
  }
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-10">
          <section>
            <div className="flex justify-between items-center mb-4 pb-2 border-b border-gray-300">
              ...
              {!shippingAddress && (
                <button onClick={() => handleOpenForm('shipping', null)} className="text-sm text-blue-600 hover:underline">
                  {t('addAddressButton')}
                </button>
              )}
            </div>
          ...
          <section>
            <div className="flex justify-between items-center mb-4 pb-2 border-b border-gray-300">
              ...
              {!billingAddress && (
                <button onClick={() => handleOpenForm('billing', null)} className="text-sm text-blue-600 hover:underline">
                  {t('addAddressButton')}
                </button>
              )}
            </div>
