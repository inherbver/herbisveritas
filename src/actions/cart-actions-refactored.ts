...
async function validateUserAuthentication(): Promise<Result<string, AuthenticationError>> {
  try {
    ...
    return Result.error(new AuthenticationError('Erreur lors de la vérification de l\'authentification'));
  }
}
...
export async function migrateAndGetCart(
  ...
  try {
    ...
    if (authenticatedUserId === guestUserId) {
      ...
      return ActionResult.ok(cartResult.success ? cartResult.data : null, 'Aucune migration nécessaire');
    }
    ...
    if (!guestCart) {
      ...
      const cartResult = await getCart();
      return ActionResult.ok(cartResult.success ? cartResult.data : null, 'Aucun panier invité trouvé');
    }
    ...
    return ActionResult.ok(finalCartResult.success ? finalCartResult.data : null, 'Migration du panier réussie');
  } catch (error) {
