  ...
  async (data: ProductFormValues): Promise<ActionResult<unknown>> => {
    ...
    >;
      ...
      if (!defaultTranslation || !defaultTranslation.name) {
        throw new ValidationError('La traduction française (fr) avec un nom de produit est requise.');
      }
...
// ===== DELETE PRODUCT =====
export const deleteProduct = withPermissionSafe("products:delete", async (productId: string): Promise<ActionResult<null>> => {
  const context = LogUtils.createUserActionContext('unknown', 'delete_product', 'products', { productId });
...
export const updateProductStatus = withPermissionSafe(
  ...
  async (data: z.infer<typeof updateProductStatusSchema>): Promise<ActionResult<unknown>> => {
    const context = LogUtils.createUserActionContext('unknown', 'update_product_status', 'products');
