...
const AddressForm: React.FC<AddressFormProps> = ({
  ...
  const processSubmit: SubmitHandler<AddressFormData> = async (data) => {
    ...
    try {
      ...
      setError('root.unexpectedError', { type: 'server', message: 'An unexpected error occurred.' });
    }
  };

  const formFields: (keyof Omit<AddressFormData, 'address_type' | 'is_default'>)[] = [
    'full_name', 'company_name', 'address_line1', 'address_line2',
    'postal_code', 'city', 'country_code', 'state_province_region', 'phone_number'
  ];

  return (
    <form onSubmit={handleSubmit(processSubmit)} className="space-y-6">
      <h3 className="text-xl font-semibold">
        {t.formTitle(addressType, isEditing)}
      </h3>
      ...
      <div className="flex items-center space-x-2 pt-2">
         <Controller
          ...
          />
        <Label htmlFor="is_default_checkbox" id="is_default_label" className="cursor-pointer select-none">
