...
export default function ClientLayout({
  ...
  // Hook pour gérer les erreurs d'authentification (refresh token, etc.)
  useAuthErrorHandler();
  ...
  const clearCartSafely = useCallback(
    (reason: string) => {
  ...
  const validateSession = useCallback(async () => {
    try {
      ...
      if (
        ...
        console.error(
          "ClientLayout: Exception during session validation:",
          error,
        );
      }
      return null;
  ...
  useEffect(() => {
    if (searchParams.get("logged_out") === "true") {
      ...
      clearCartSafely(
        "Detected logged_out=true URL parameter via useSearchParams",
      );

      // Supprimer le paramètre de l'URL pour éviter le conflit avec la reconnexion
  ...
  useEffect(() => {
    // Configurer la suppression d'erreur pour les extensions
    ...
    const checkInitialSession = async () => {
      ...
      console.log(
        "ClientLayout: Initial session check:",
        ...
        switch (event) {
                ...
                setTimeout(async () => {
                  try {
                    ...
                    if (!validatedSession && session) {
                      console.log(
                        "ClientLayout: Session validation failed, clearing cart",
                      );
                      clearCartSafely(
                        "Session validation failed after initial session",
                      );
                    }
                  } catch (error) {
                    console.warn(
          ...
          case "TOKEN_REFRESHED":
            if (!session && lastSessionState.current) {
              console.log(
                "ClientLayout: Token refresh failed, session expired",
              );
