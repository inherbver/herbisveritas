...
function AddToCartButton({
  ...
  product: Product | ReturnType<typeof ProductFactory.simple>;
  variant?: any;
  quantity?: number;
  disabled?: boolean;
  className?: string;
...
describe("AddToCartButton - Tests Composant React (Phase 3.3)", () => {
  ...
  describe("États et interactions de base", () => {
    it("should render add to cart button for available product", () => {
      ...
      const button = screen.getByRole("button", {
        name: /ajouter produit test au panier/i,
      });
    ...
    it("should handle click interaction with loading state", async () => {
      ...
      // Act
      const button = screen.getByRole("button", {
        name: /ajouter produit interactif au panier/i,
      });
  ...
  describe("Accessibilité et UX", () => {
    it("should have proper accessibility attributes", () => {
      ...
      expect(button).toHaveAttribute(
        "aria-label",
        "Ajouter Produit Accessible au panier",
      );
  ...
  describe("Gestion des erreurs", () => {
    it("should handle network errors gracefully", async () => {
      ...
      function FailingAddToCartButton({
        product,
      }: {
        product: Product | ReturnType<typeof ProductFactory.simple>;
      }) {
      ...
      const { user } = renderWithProviders(
        <FailingAddToCartButton product={product} />,
      );
  ...
  describe("Tests de performance", () => {
    it("should not re-render unnecessarily", () => {
      ...
      function TrackingAddToCartButton({
        product,
      }: {
        product: Product | ReturnType<typeof ProductFactory.simple>;
      }) {
      ...
      const { rerender } = renderWithProviders(
        <TrackingAddToCartButton product={product} />,
      );
  ...
  describe("Tests d'intégration avec le store", () => {
    it("should update cart count when item is added", async () => {
      ...
      function CartWithButton() {
        const [cartItemCount, setCartItemCount] = React.useState(
          cart.items.length,
        );
      ...
      const { user: userEvent } = renderWithProviders(<CartWithButton />, {
        user,
        cart,
      });
