...
function AddressForm({
    ...
    try {
      ...
      if (!result.isValid) {
        setErrors((prev) => ({
          ...prev,
          colissimo:
            "Adresse non trouvée par Colissimo. Vérifiez les informations.",
        }));
      } else {
        ...
        if (result.normalizedAddress) {
          setFormData((prev) => ({
            ...
            postal_code:
              result.normalizedAddress.postalCode || prev.postal_code,
          }));
        }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const isValid = Object.keys(formData).every((key) =>
      validateField(key, formData[key as keyof AddressFormData]),
    );
  ...
  return (
    <form onSubmit={handleSubmit} noValidate>
      ...
      <div className="form-group">
        <label htmlFor={`${type}-line2`}>Complément d'adresse</label>
        <input
          id={`${type}-line2`}
          name="line2"
          type="text"
      ...
      <div className="form-row">
        <div className="form-group">
          ...
          <input
            ...
            aria-describedby={
              errors.postal_code ? `${type}-postal_code-error` : undefined
            }
            placeholder="75001"
            maxLength={5}
          />
...
describe("AddressForm - Tests Composant React (Phase 3.3)", () => {
  ...
  describe("Rendu et états de base", () => {
    it("should render shipping address form with all required fields", () => {
      ...
      // Assert
      expect(
        screen.getByRole("heading", { name: /adresse de livraison/i }),
      ).toBeInTheDocument();
      expect(screen.getByLabelText(/^Adresse/)).toBeInTheDocument();
      expect(
        screen.getByLabelText(/complément d'adresse/i),
      ).toBeInTheDocument();
    ...
    it("should render billing address form when type is billing", () => {
      ...
      // Assert
      expect(
        screen.getByRole("heading", { name: /adresse de facturation/i }),
      ).toBeInTheDocument();
    });
    ...
    it("should populate form with initial data", () => {
      ...
      // Assert
      expect(
        screen.getByDisplayValue("123 rue de la Paix"),
      ).toBeInTheDocument();
  ...
  describe("Validation Colissimo", () => {
    it("should show Colissimo validation button for French addresses", async () => {
      ...
      await testHelpers.clearAndType(
        user,
      ...
      await testHelpers.clearAndType(
        user,
        screen.getByLabelText(/code postal.*\*/),
        "75001",
      );
      await testHelpers.clearAndType(
        user,
        screen.getByLabelText(/ville.*\*/),
        "Paris",
      );
      ...
      await waitFor(() => {
        expect(
          screen.getByRole("button", { name: /vérifier avec colissimo/i }),
        ).toBeInTheDocument();
      });
    });
    ...
    it("should call Colissimo API and handle successful validation", async () => {
      ...
      const validateButton = await screen.findByRole("button", {
        name: /vérifier avec colissimo/i,
      });
    ...
    it("should handle Colissimo validation errors", async () => {
      ...
      await waitFor(() => {
        expect(screen.getByRole("alert")).toHaveTextContent(
          "Adresse non trouvée par Colissimo",
        );
      });
    });
  });
  ...
  describe("Soumission du formulaire", () => {
    ...
    it("should show loading state during submission", async () => {
      ...
      await waitFor(
        () => {
          expect(
            screen.getByRole("button", { name: /enregistrer l'adresse/i }),
          ).toBeInTheDocument();
        },
        { timeout: 1000 },
      );
  ...
  describe("Accessibilité", () => {
    it("should have proper ARIA attributes for form validation", async () => {
      ...
      await waitFor(() => {
        expect(addressField).toHaveAttribute("aria-invalid", "true");
        expect(addressField).toHaveAttribute(
          "aria-describedby",
  ...
  describe("Callback de validation", () => {
    it("should call onValidation callback when form validity changes", async () => {
      ...
      const { user } = renderWithProviders(
        <AddressForm onValidation={mockOnValidation} />,
      );
