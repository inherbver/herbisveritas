...
const Footer = React.forwardRef<HTMLElement, FooterProps>(
  ({ className, ...props }, ref) => {
    ...
    return (
      <footer
        ref={ref}
        className={cn('bg-muted/40 border-t',
          className
        )}
        {...props}
      >
        <div className="container flex flex-col items-center justify-between gap-4 py-10 md:h-24 md:flex-row md:py-0">
          <div className="flex flex-col items-center gap-4 px-8 md:flex-row md:gap-2 md:px-0">
            <Logo />
            <Text as="p" className="text-center text-sm leading-loose text-muted-foreground md:text-left">
          ...
          <nav className="flex flex-wrap justify-center gap-x-6 gap-y-2 text-sm text-muted-foreground md:justify-end">
            {/* Exemple de liens secondaires - à remplacer */}
