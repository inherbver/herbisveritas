...
export class CartDomainService {
  ...
  async addItemToCart(
    ...
    const context = LogUtils.createUserActionContext(userId, "add_item_to_cart", "cart", {
      productId,
      quantity,
    });
    ...
    try {
      ...
      try {
        ...
        return Result.error(
          new ValidationError(error instanceof Error ? error.message : "Quantité invalide")
        );
      }
      ...
      if (addResult.isError()) {
        LogUtils.logOperationError(
          "CartDomainService.addItemToCart",
          addResult.getError(),
          context
        );
  ...
  async removeItemFromCart(
    ...
    const context = LogUtils.createUserActionContext(userId, "remove_item_from_cart", "cart", {
      itemId,
    });
  ...
  async updateItemQuantity(
    ...
    const context = LogUtils.createUserActionContext(userId, "update_item_quantity", "cart", {
      itemId,
      newQuantity,
    });
  ...
  async mergeCarts(
    ...
    const context = LogUtils.createUserActionContext(toUserId, "merge_carts", "cart", {
      fromUserId,
    });
