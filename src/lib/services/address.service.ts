...
import { addAddress, updateAddress, deleteAddress, getUserAddresses } from '@/actions/addressActions';
...
export class AddressService {
   ...
   */
  async getUserAddresses(userId: string, filters?: AddressFilters): Promise<Result<Address[], Error>> {
    ...
    try {
      if (isRepositoryEnabled('USE_ADDRESS_REPOSITORY')) {
        ...
        LogUtils.logOperationWarning('getUserAddresses', 'Repository failed, falling back to legacy', context);
      }
   ...
   */
  async createAddress(userId: string, addressData: CreateAddressData, locale: string = 'fr'): Promise<Result<Address, Error>> {
    ...
    try {
      if (isRepositoryEnabled('USE_ADDRESS_REPOSITORY')) {
        ...
        const result = await this.repository.createAddress(userId, normalizedResult.getValue()!);
        ...
        LogUtils.logOperationWarning('createAddress', 'Repository failed, falling back to legacy', context);
      }
      ...
      return Result.failure(new Error(legacyResult.error || 'Failed to create address via legacy system'));
   ...
   */
  async updateAddress(addressId: string, userId: string, addressData: UpdateAddressData, locale: string = 'fr'): Promise<Result<Address, Error>> {
    ...
    try {
      if (isRepositoryEnabled('USE_ADDRESS_REPOSITORY')) {
        ...
        const result = await this.repository.updateAddress(addressId, userId, normalizedResult.getValue()!);
        ...
        LogUtils.logOperationWarning('updateAddress', 'Repository failed, falling back to legacy', context);
      }
      ...
      return Result.failure(new Error(legacyResult.error || 'Failed to update address via legacy system'));
  ...
  async deleteAddress(addressId: string, userId: string): Promise<Result<void, Error>> {
    ...
    try {
      if (isRepositoryEnabled('USE_ADDRESS_REPOSITORY')) {
        ...
        LogUtils.logOperationWarning('deleteAddress', 'Repository failed, falling back to legacy', context);
      }
      ...
      return Result.failure(new Error(legacyResult.error || 'Failed to delete address via legacy system'));
  ...
  async getAddressesByType(userId: string, type: AddressType): Promise<Result<Address[], Error>> {
    ...
    try {
      ...
      if (allAddressesResult.isSuccess()) {
        const filtered = allAddressesResult.getValue()!.filter(addr => addr.address_type === type);
        LogUtils.logOperationSuccess('getAddressesByType', { 
          ...context, 
          source: 'filtered',
   ...
   */
  async getDefaultAddress(userId: string, type: AddressType): Promise<Result<Address | null, Error>> {
   ...
   * Valider une adresse
   */
  async validateAddress(addressData: CreateAddressData | UpdateAddressData): Promise<Result<void, Error>> {
