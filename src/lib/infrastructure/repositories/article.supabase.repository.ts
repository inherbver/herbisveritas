...
export class ArticleSupabaseRepository implements IArticleRepository {
   ...
   */
  private async generateSlugFromTitle(
    title: string,
    locale: string,
    excludeId?: string
  ): Promise<string> {
  ...
  async findBySlug(
    slug: string,
    locale = "fr"
  ): Promise<Result<ArticleWithCurrentTranslation | null, Error>> {
    ...
    try {
      ...
      const translation = data.translations.find(
        (t: Database["public"]["Tables"]["article_translations"]["Row"]) => t.locale === locale
      );
      if (!translation) {
        LogUtils.logOperationEnd("findBySlug", context, { found: false });
        return Result.success(null);
  ...
  async findByIdWithTranslations(
    id: string
  ): Promise<Result<ArticleWithTranslations | null, Error>> {
    const context = LogUtils.createOperationContext(
      "findByIdWithTranslations",
      this.repositoryName
    );
  ...
  async findPublishedArticles(
  ...
  ): Promise<Result<PaginatedArticles, Error>> {
    ...
    try {
      ...
      if (filters.search) {
        query = query.or(
          `article_translations.title.ilike.%${filters.search}%,article_translations.content.ilike.%${filters.search}%,article_translations.excerpt.ilike.%${filters.search}%`
        );
      }
      ...
      const articles: ArticleWithCurrentTranslation[] = data.map(
        (
  ...
  async updateArticleStatus(
    ...
    try {
      const updateData: Database["public"]["Tables"]["articles"]["Update"] = { status };
  ...
  async createTranslation(
    articleId: string,
    translationData: CreateArticleTranslationData
  ): Promise<Result<ArticleTranslation, Error>> {
    const context = LogUtils.createOperationContext("createTranslation", this.repositoryName);
    LogUtils.logOperationStart("createTranslation", context, {
      articleId,
  ...
  async updateTranslation(
    articleId: string,
    locale: string,
    translationData: UpdateArticleTranslationData
  ): Promise<Result<ArticleTranslation, Error>> {
    ...
    try {
      ...
      if (translationData.title) {
        updateData.slug = await this.generateSlugFromTitle(
  ...
  async findTranslation(
    articleId: string,
    locale: string
  ): Promise<Result<ArticleTranslation | null, Error>> {
  ...
  async searchArticles(
    ...
    locale = "fr"
  ): Promise<Result<ArticleWithCurrentTranslation[], Error>> {
  ...
  async getFeaturedArticles(
    limit = ARTICLE_CONFIG.FEATURED_ARTICLES_LIMIT,
    locale = "fr"
  ): Promise<Result<ArticleWithCurrentTranslation[], Error>> {
    const searchParams: ArticleSearchParams = {
  ...
  async findRelatedArticles(
    articleId: string,
    limit = ARTICLE_CONFIG.RELATED_ARTICLES_LIMIT,
    locale = "fr"
  ): Promise<Result<ArticleWithCurrentTranslation[], Error>> {
    ...
    try {
      ...
      const { data, error } = await this.client
        ...
        .neq("id", articleId)
        .or(
          `tags.ov.{${sourceArticle.tags.join(",")}},categories.ov.{${sourceArticle.categories.join(",")}}`
        )
  ...
  async validateArticleData(
    ...
    try {
      ...
      if (
        "status" in articleData &&
        articleData.status === "scheduled" &&
        !articleData.scheduled_at
      ) {
        return Result.failure(
          new ValidationError("scheduled_at is required when status is scheduled")
        );
      }
      ...
      if (
        ...
        new Date(articleData.published_at) > new Date()
      ) {
        return Result.failure(
          new ValidationError("published_at cannot be in the future unless status is scheduled")
        );
      }
  ...
  async validateTranslations(
    translations: CreateArticleTranslationData[]
  ): Promise<Result<void, Error>> {
    ...
    try {
      ...
      for (const translation of translations) {
        if (!translation.title?.trim()) {
          return Result.failure(
            new ValidationError(`Title is required for locale ${translation.locale}`)
          );
        }
        if (!translation.content?.trim()) {
          return Result.failure(
            new ValidationError(`Content is required for locale ${translation.locale}`)
          );
        }
      }
  ...
  async generateSEOData(articleId: string, locale: string): Promise<Result<ArticleSEOData, Error>> {
    ...
    try {
      ...
      if (!translation) {
        return Result.failure(
          new NotFoundError(`Translation not found for article ${articleId} in locale ${locale}`)
        );
      }
  ...
  async updateSEOMetadata(
    articleId: string,
    locale: string,
    seoData: Partial<ArticleSEOData>
  ): Promise<Result<ArticleTranslation, Error>> {
  ...
  async recordView(
    articleId: string,
    visitorData?: { ip: string; userAgent: string; referrer?: string }
  ): Promise<Result<void, Error>> {
    ...
    try {
      ...
      const { error: updateError } = await this.adminClient
        .from("articles")
        .update({
          view_count: this.adminClient.sql`view_count + 1`,
      ...
      if (visitorData) {
        const { error: analyticsError } = await this.adminClient.from("article_views").insert({
          article_id: articleId,
  ...
  async getArticleAnalytics(
    ...
    _dateFrom?: string,
    _dateTo?: string
  ): Promise<Result<ArticleAnalytics, Error>> {
  ...
  async getMostViewedArticles(
    limit = 10,
    locale = "fr",
    _period: "day" | "week" | "month" | "all" = "all"
  ): Promise<Result<ArticleWithCurrentTranslation[], Error>> {
    ...
    try {
      ...
      if (data) {
        ...
        for (const article of data) {
          stats[article.status as keyof typeof stats] =
            (stats[article.status as keyof typeof stats] as number) + 1;
  ...
  async cleanupUnusedTaxonomies(): Promise<
    Result<{ removedTags: number; removedCategories: number }, Error>
  > {
  ...
  async exportArticles(filters?: ArticleFilters): Promise<Result<string, Error>> {
    ...
    try {
      ...
      LogUtils.logOperationEnd("exportArticles", context, {
        exported: result.data.articles.length,
      });
