...
import { ContainerConfiguration, resolveService, resolveAdminService } from '../../container/container.config';
...
import type { IArticleRepository } from '@/lib/domain/interfaces/article.repository.interface';
...
describe('Phase 3 Integration Tests - Repository Pattern', () => {
  ...
  describe('Repository Resolution', () => {
    ...
    it('should resolve OrderRepository from server container', async () => {
      const orderRepository = await resolveService<IOrderRepository>(SERVICE_TOKENS.ORDER_REPOSITORY);
    ...
    it('should resolve ArticleRepository from server container', async () => {
      const articleRepository = await resolveService<IArticleRepository>(SERVICE_TOKENS.ARTICLE_REPOSITORY);
      
      expect(articleRepository).toBeDefined();
      expect(typeof articleRepository.create).toBe('function');
      expect(typeof articleRepository.findBySlug).toBe('function');
    });
