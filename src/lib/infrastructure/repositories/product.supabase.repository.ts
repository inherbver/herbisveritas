...
export class ProductSupabaseRepository
  extends BaseSupabaseRepository<Product, CreateProductData, UpdateProductData>
  implements IProductRepository
{
  ...
  private async findBySlugUncached(
    slug: string,
    locale?: string
  ): Promise<Result<ProductWithCurrentTranslation | null, Error>> {
      ...
      if (!data) {
        ...
        if (locale && locale !== this.defaultLocale) {
          LogUtils.logOperationInfo(
            "findBySlugUncached",
            `Fallback to default locale: ${this.defaultLocale}`,
            context
          );
    ...
    try {
      ...
      if (error && error.code !== "PGRST116") {
        ...
        return Result.failure(
          new DatabaseError(`Error finding product with translations: ${error.message}`)
        );
      }
  ...
  private async findActiveProductsUncached(
    params: ProductSearchParams
  ): Promise<Result<PaginatedProducts, Error>> {
    const context = LogUtils.createOperationContext(
      "findActiveProductsUncached",
      "product-repository"
    );
  ...
  async adjustStock(
    update: StockUpdate
  ): Promise<Result<{ product: Product; movement: StockMovement }, Error>> {
  ...
  async validateProductData(
    ...
    try {
      ...
      if ("slug" in productData && productData.slug && !/^[a-z0-9-]+$/.test(productData.slug)) {
        return Result.failure(
          new ValidationError("Slug must contain only lowercase letters, numbers, and hyphens")
        );
      }
  ...
  async isSlugAvailable(slug: string, excludeProductId?: string): Promise<Result<boolean, Error>> {
    ...
    try {
      let query = this.supabase.from(this.tableName).select("id").eq("slug", slug);
      ...
      if (error && error.code !== "PGRST116") {
        ...
        return Result.failure(
          new DatabaseError(`Error checking slug availability: ${error.message}`)
        );
      }
  ...
  async createTranslation(
    _productId: string,
    _translationData: CreateProductTranslationData
  ): Promise<Result<ProductTranslation, Error>> {
  ...
  async updateTranslation(
    _productId: string,
    _locale: string,
    _translationData: UpdateProductTranslationData
  ): Promise<Result<ProductTranslation, Error>> {
    return Result.failure(new Error("Not implemented"));
  }
  ...
  async findTranslation(
    _productId: string,
    _locale: string
  ): Promise<Result<ProductTranslation | null, Error>> {
    return Result.failure(new Error("Not implemented"));
  }
  ...
  async searchProducts(
    ...
    _locale?: string
  ): Promise<Result<ProductWithCurrentTranslation[], Error>> {
    return Result.failure(new Error("Not implemented"));
  }
  ...
  async validateTranslations(
    _translations: CreateProductTranslationData[]
  ): Promise<Result<void, Error>> {
    return Result.failure(new Error("Not implemented"));
  }
