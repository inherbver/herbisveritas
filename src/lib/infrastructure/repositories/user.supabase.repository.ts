...
export class UserSupabaseRepository implements IUserRepository {
    ...
    try {
      ...
      if (profilesError) {
        ...
        return Result.failure(
          new DatabaseError(`Error finding profiles: ${profilesError.message}`)
        );
      }
  ...
  async updateUserRole(
    ...
    const context = LogUtils.createOperationContext("updateUserRole", "user-repository");
    LogUtils.logOperationStart("updateUserRole", { ...context, userId, role });
    ...
    try {
      ...
      if (error) {
        LogUtils.logOperationError("updateUserRole", error, context);
        if (error.code === "PGRST116") {
          return Result.failure(new NotFoundError(`Profile not found for user ${userId}`));
        }
        return Result.failure(new DatabaseError(`Error updating user role: ${error.message}`));
      }

      LogUtils.logOperationSuccess("updateUserRole", { ...context, profileId: data.id });
      return Result.success(data);
    } catch (error) {
      LogUtils.logOperationError("updateUserRole", error, context);
      return this.handleError(error);
  ...
  async findAdminUsers(): Promise<Result<AdminUserData[], Error>> {
    ...
    try {
      ...
      if (profilesError) {
        ...
        return Result.failure(
          new DatabaseError(`Error finding admin profiles: ${profilesError.message}`)
        );
      }
  ...
  async isEmailTaken(email: string, excludeUserId?: string): Promise<Result<boolean, Error>> {
    ...
    try {
      ...
      const existingUser = data.users.find((u) => u.email === email && u.id !== excludeUserId);
