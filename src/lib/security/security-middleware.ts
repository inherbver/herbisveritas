...
export class SecurityMiddleware {
  ...
  checkRateLimit(endpoint: string, context: SecurityContext): Result<void, RateLimitError> {
    ...
    const key = config.keyGenerator ? config.keyGenerator(context) : `${endpoint}:${context.ip}`;
    ...
    if (entry.count > config.maxRequests) {
      ...
      return Result.error(
        new RateLimitError(`Trop de requêtes. Réessayez dans ${resetTime} secondes.`)
      );
    }
  ...
  checkAuthorization(
    ...
    if (!context.userRole || !rule.roles.includes(context.userRole)) {
      ...
      return Result.error(new AuthorizationError("Permissions insuffisantes pour cette action"));
    }
    ...
    if (rule.conditions && !rule.conditions(context)) {
      ...
      return Result.error(new AuthorizationError("Conditions d'autorisation non remplies"));
    }
  ...
  sanitizeInput<T extends Record<string, unknown>>(
    ...
    try {
      ...
      return Result.ok(sanitized as T);
    } catch (error) {
      return Result.error(
        new ValidationError("Erreur lors de la sanitisation des données", undefined, {
          originalError: error,
        })
      );
    }
  ...
  async validateRequestSignature(
    ...
    try {
      ...
      return Result.error(
        new ValidationError("Erreur lors de la validation de signature", undefined, {
          originalError: error,
        })
      );
    }
  ...
  detectSuspiciousActivity(context: SecurityContext): Result<void, BusinessError> {
    ...
    if (suspicious.some(Boolean)) {
      ...
      return Result.error(new BusinessError("Activité suspecte détectée. Requête bloquée."));
    }
...
export const SecurityUtils = {
  ...
  validatePasswordStrength(password: string): Result<void, ValidationError> {
    ...
    if (errors.length > 0) {
      return Result.error(new ValidationError(`Mot de passe trop faible: ${errors.join(", ")}`));
    }
