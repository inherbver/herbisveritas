...
export function createMockSupabaseClient(options: MockSupabaseOptions = {}) {
  ...
  const queryMethods: any = {
    ...
    // Filtres
    eq: jest.fn(function (this: any, column: string, value: unknown) {
    ...
    then: jest.fn((callback: (result: unknown) => unknown) => {
  ...
  const mockClient = {
    ...
    from: jest.fn((table: string) => {
      ...
      if (customResponses[table]) {
        tableQueryMethods.then = jest
          .fn()
          .mockResolvedValue(customResponses[table]);
      }
