...
interface RenderWithProvidersOptions {
  ...
  mockSupabaseClient?: unknown;

  // Configuration user-event
  _userEventOptions?: Parameters<typeof userEvent.setup>[0];
}

interface ExtendedRenderResult extends Omit<RenderResult, "rerender"> {
...
export function renderWithProviders(
  ...
  // Setup user-event
  const userEventInstance = userEvent.setup(_userEventOptions);
...
export function renderServerComponent(
  ui: React.ReactElement,
  options: Omit<RenderWithProvidersOptions, "_userEventOptions"> = {},
) {
  const { _userEventOptions, ...restOptions } =
    options as RenderWithProvidersOptions;
...
export function setupIntegrationTest(options: RenderWithProvidersOptions = {}) {
  ...
  return {
    render: (
      ui: React.ReactElement,
      extraOptions: RenderWithProvidersOptions = {},
